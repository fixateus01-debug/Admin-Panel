...
//   /* ---------------- SUBMIT PLAN & UPDATE EXAMS/TESTS/PYQS COLLECTIONS ---------------- */
...
//         await updateDoc(doc(db, "subscriptionPlans", oldPlan.id), payload);
//         planId = oldPlan.id;

//         /* --------- DETECT CHANGES --------- */

//         if (oldPlan.price !== formData.price) {
//           await logActivity({
//             actionType: "PLAN_PRICE_CHANGED",
//             description: `Price changed for ${formData.name} from ₹${oldPlan.price} to ₹${formData.price}`,
//             entityId: planId,
//             entityType: "subscriptionPlan",
//           });
//         }

//         if (oldPlan.durationDays !== formData.durationDays) {
//           await logActivity({
//             actionType: "PLAN_DURATION_CHANGED",
//             description: `Duration changed for ${formData.name} from ${oldPlan.durationDays} days to ${formData.durationDays} days`,
//             entityId: planId,
//             entityType: "subscriptionPlan",
//           });
//         }

//         if (JSON.stringify(oldPlan.features) !== JSON.stringify(formData.features)) {
//           await logActivity({
//             actionType: "PLAN_FEATURES_UPDATED",
//             description: `Features updated for ${formData.name}`,
//             entityId: planId,
//             entityType: "subscriptionPlan",
//           });
//         }

//         if (oldPlan.isActive !== formData.isActive) {
//           await logActivity({
//             actionType: "PLAN_STATUS_CHANGED",
//             description: `${formData.name} was ${formData.isActive ? "Activated" : "Deactivated"}`,
//             entityId: planId,
//             entityType: "subscriptionPlan",
//           });
//         }

//         if (JSON.stringify(oldPlan.examsIncluded) !== JSON.stringify(formData.examsIncluded)) {
//           await logActivity({
//             actionType: "PLAN_EXAMS_UPDATED",
//             description: `Exam/Test/PYQ mapping updated for ${formData.name}`,
//             entityId: planId,
//             entityType: "subscriptionPlan",
//           });
//         }
...
//           description: `Updated subscription plan: ${formData.name}`,
//           entityId: planId,
//           entityType: "subscriptionPlan",
//         });
...
//         const newPlanRef = await addDoc(collection(db, "subscriptionPlans"), {
//           ...payload,
...
//           description: `Created subscription plan: ${formData.name} (₹${formData.price})`,
//           entityId: planId,
//           entityType: "subscriptionPlan",
//         });
//       }
...
export default function SubscriptionPlans() {
  ...
  // /* ---------------- SUBMIT PLAN & APPLY TO TESTS/PYQS ---------------- */
  // const handleSubmit = async (e) => {
  //   e.preventDefault();
  ...
  //       await updateDoc(
  //         doc(db, "subscriptionPlans", oldPlan.id),
  //         payload
  //       );
  ...
  //     /* ================= APPLY PLAN TO TESTS & PYQS ================= */

  //     for (const examId in formData.examsIncluded) {

  //       const { tests = [], pyqs = [] } = formData.examsIncluded[examId];
  ...
  //           } catch (err) {
  //             // ignore if chapter not under this subject
  //           }
  //         }
  //       }
  //     }
